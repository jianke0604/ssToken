...
def _unwrap_if_fsdp(mod):
    return mod.module if isinstance(mod, FSDP) else mod
...
class LayerCache:
    def __init__(self):
        ...
        self.attn_mask = None
        self.pos_emb = None        # (cos, sin)
        self.cache_position = None
        self.position_ids = None   
    def clear(self):
        self.__init__()

def register_layer_pre_hook(model, layer_idx:int, cache: LayerCache):
    ...
    def pre_hook(mod, args, kwargs):
        ...
        # )
        cache.hidden_states  = kwargs.get("hidden_states",  args[0] if len(args)>0 else None)
        cache.attn_mask      = kwargs.get("attention_mask", args[1] if len(args)>1 else None)
        cache.position_ids   = kwargs.get("position_ids",   args[2] if len(args)>2 else None)
        cache.cache_position = kwargs.get("cache_position", None)
        cache.pos_emb        = kwargs.get("position_embeddings", None)   # (cos, sin) 
        return None
...
@torch.no_grad()
def compute_attn_mean_from_layer_cache(cache: LayerCache):
    layer = cache.layer                          # LlamaDecoderLayer
    inner = _unwrap_if_fsdp(layer) 
    hs, mask, pos_emb, cpos = cache.hidden_states, cache.attn_mask, cache.pos_emb, cache.cache_position
    dev = next(inner.parameters()).device
    hs = hs.to(dev); mask = None if mask is None else mask.to(dev)
    pos_emb = (pos_emb[0].to(dev), pos_emb[1].to(dev)); cpos = None if cpos is None else cpos.to(dev)

    ctx = FSDP.summon_full_params(layer, writeback=False, recurse=True) if isinstance(layer, FSDP) else nullcontext()
    with ctx:
        hs_norm = inner.input_layernorm(hs)
        _, attn_w = inner.self_attn(
    ...
    return attn_w.mean(dim=1)  # [B,S,S]
...
def filter_tokens_with_attention_loss(
    logits_b: torch.Tensor,     # [seq_len, vocab_size]
    labels_b: torch.Tensor,     # [seq_len]
    attn_b: torch.Tensor,       # [seq_len, seq_len]
    ...
    seq_len = labels_b.size(0)

    prompt_len = (labels_b == -100).sum().item()
    resp_len = seq_len - prompt_len
    if resp_len <= 0:
        return labels_b  

    shift_logits = logits_b[:-1]            # [seq_len-1, vocab]
    shift_labels = labels_b[1:]             # [seq_len-1]
    loss_fct = torch.nn.CrossEntropyLoss(reduction="none")
    per_token_loss = loss_fct(shift_logits, shift_labels)  # [seq_len-1]

    per_token_loss = torch.cat([torch.zeros(1, device=device), per_token_loss], dim=0)  # [seq_len]

    resp_loss = per_token_loss[prompt_len:]  # [resp_len]

    resp2prompt_scores = attn_b[prompt_len:, :prompt_len].sum(dim=-1)  # [resp_len]
    # print(f"len of per_token_loss: {len(resp_loss)}")
    # print(resp_loss)
    # print(f"len of resp2prompt_scores: {len(resp2prompt_scores)}")
    # print(resp2prompt_scores)


    if mode == "combine" and ref_losses is not None:
        ref_resp_loss = ref_losses[prompt_len:]
        diff = ref_resp_loss - resp_loss
        diff_norm = (diff - diff.min()) / (diff.max() - diff.min() + 1e-8)  # [resp_len]
        # print(diff)

        combined = ratio * diff_norm + (1 - ratio) * resp2prompt_scores  # [resp_len]
    else:
        combined = resp2prompt_scores  # [resp_len]

    k = math.ceil(resp_len * data_prop)
    keep_idx = torch.topk(combined, k=k, largest=True).indices  
    keep_mask = torch.zeros(resp_len, dtype=torch.bool, device=device)
    keep_mask[keep_idx] = True

    new_labels = labels_b.clone()
    resp_positions = torch.arange(prompt_len, seq_len, device=device)
    new_labels[resp_positions[~keep_mask]] = -100  
...
def parse_args():
    ...
    parser.add_argument(
        '--reduce_loss',
        default='mean',
        choices=['mean', 'sum', 'selection'],
        help='How to reduce loss over tokens.',
    )
    parser.add_argument(
        '--wandb_entity', 
        type=str,
    ...
    parser.add_argument(
        "--ratio", type=float, default=0.5, help="The weight ratio between loss and attention score"
    )
    parser.add_argument(
...
def main():
    ...
    with accelerator.main_process_first():
        ...
        if args.token_select_pattern == 'token_cleaning': 
            ...
            selected_labels = torch.load(args.label_path + f"token_cleaning_labels.pt")           
    ...
    logger.info(f"  loss-attention ratio = {args.ratio}")
    ...
    ref_token_losses = torch.load(
        "your_path_to_base_model_losses.pt"
    )
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        # in early stage, only use attention to select tokens
        pre_ratio=0
        pre_index=10000
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                indices = batch["idx"]
                # print(f"Processing batch with indices: {indices}")
                # indices = [0 for _ in range(batch["labels"].size(0))]
                layer_idx = -1
                cache = LayerCache()
                hooks = register_layer_pre_hook(model, layer_idx, cache)
                outputs = model(
                ...
                if args.reduce_loss == 'mean':
                    ...
                    # remove_hooks(hooks)
                    with torch.no_grad():
                        attn_mean = compute_attn_mean_from_layer_cache(cache)  # [B,S,S]
                    # print(f"attn_mean_hook : {attn_mean_hook.shape}, {attn_mean_hook}")
                    for b in range(attn_mean.size(0)):
                        seq_len = batch["attention_mask"][b].sum().item()   

                        attn_b = attn_mean[b, :seq_len, :seq_len]           # [seq_len, seq_len]
                        labels_b = batch["labels"][b, :seq_len]
                        logits_b = outputs.logits[b, :seq_len]  # [seq_len, vocab_size]
                        sample_idx = indices[b].item()
                        if sample_idx >= pre_index:
                            pre_ratio = args.ratio
                        ref_losses_b = torch.tensor(ref_token_losses[sample_idx], device=logits.device)
                        # print(f"seq_len: {seq_len}, ref_losses_b: {ref_losses_b.shape}")
                        filtered_labels_b = filter_tokens_with_attention_loss(
                            logits_b, labels_b, attn_b, ref_losses_b,
                            data_prop=args.data_prop, ratio=pre_ratio, mode="combine"
                        )
                        new_labels[b, :seq_len] = filtered_labels_b
                    # print(f"Layer {layer_idx} attention matrices shape: {[attn.shape for attn in attn_matrices]}")
                    # print(f"Layer {layer_idx} attention shape: {attn_mean.shape}")
